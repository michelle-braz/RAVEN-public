...
_PLACEHOLDERS = {
    ...
    "ts": "<ts>",
    "num": "<num>",
    # A recognized HTTP 5xx status code carries a distinct signal from any other
    # standalone number (ports, PIDs, counts) — it must not collapse into <num>.
    "httperr": "<httperr>",
}
...
_SCRUBBERS: tuple[tuple[re.Pattern[str], str], ...] = (
    ...
    # HTTP 5xx status code near the literal word "http" — must run before the
    # generic number catch-all below, or the code silently collapses into <num>
    # and becomes indistinguishable from a port, PID, or retry count.
    (re.compile(r"\bhttp\b.{0,20}?\b5\d{2}\b", re.I), _PLACEHOLDERS["httperr"]),
    # Any remaining standalone numbers (ports, counts, pids, durations).
    # Keep a literal "0%" in place: it is explicit absence, not cardinality.
    (re.compile(r"\b(?!0\b(?=\s*%))\d+\b"), _PLACEHOLDERS["num"]),
)
...
def _fold_diacritics(text: str) -> str:
    """Strip combining accent marks (e.g. 'não' -> 'nao') so _TOKEN_RE's ASCII
    a-z class doesn't fragment accented words — language-agnostic, not PT-specific."""
    return "".join(c for c in unicodedata.normalize("NFKD", text) if not unicodedata.combining(c))


def _scrub(message: str) -> tuple[str, int]:
    text = _fold_diacritics(message.lower())
    stripped = 0
