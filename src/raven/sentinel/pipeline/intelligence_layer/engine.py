...
_THREAT_LEXICON: dict[str, float] = {
    ...
    # ── Portuguese equivalents — same concept, same weight tier as the English
    # entry above. The lexicon was English-only, so Portuguese-language events
    # (RAVEN's pilot audience) scored lexical=0.0 regardless of real content.
    "erro": 0.45,        # mirrors "error"
    "erros": 0.50,       # mirrors "errors"
    "falha": 0.55,       # mirrors "failure"
    "falhas": 0.55,      # mirrors "failures"
    "indisponivel": 0.90,        # mirrors "unavailable" (accent-folded from "indisponível")
    "indisponibilidade": 0.85,   # mirrors "outage"
    # ── Structural signal ─────────────────────────────────────────────────────
    # Placeholder token emitted by the normalizer for a recognized HTTP 5xx status
    # code (see normalizer._SCRUBBERS / _PLACEHOLDERS["httperr"]). Weighted
    # moderately: a single 5xx mention alone must not be enough to reach
    # HIGH/CRITICAL without other corroborating signal.
    "<httperr>": 0.20,
}

# Negation markers: when a lexicon word is immediately preceded by one of these
# in the normalized text, it is excluded from scoring. Adjacent-word only (not a
# wider window) to stay conservative. "no" is deliberately excluded — in
# Portuguese it is commonly a preposition contraction, not a negator, so
# including it would misfire.
_NEGATION_RE = re.compile(r"\b(?:not|nao|nenhum|nenhuma|sem|nunca)\s+([a-z]+)\b", re.I)
_ZERO_ERROR_RATE_RE = re.compile(
    r"(?:0\s*%|0\s+percent|zero\s+percent)\s+(errors?|erros?|failures?|falhas?)",
    re.I,
)
...
class RiskEngine:
    ...
    def assess(
        ...
        negated = frozenset(m.group(1) for m in _NEGATION_RE.finditer(signal.normalized_text))
        negated = negated | frozenset(
            m.group(1) for m in _ZERO_ERROR_RATE_RE.finditer(signal.normalized_text)
        )
        ...
        features = ImmutableFeatureVector(
            values={
                "lexical": self._lexical_factor(signal.tokens, negated),
    ...
    @staticmethod
    def _lexical_factor(tokens: tuple[str, ...], negated: frozenset[str] = frozenset()) -> float:
        hits = sorted(
            (
                _THREAT_LEXICON[t]
                for t in tokens
                if t in _THREAT_LEXICON and t.strip("<>") not in negated
            ),
            reverse=True,
        )
        if not hits:
