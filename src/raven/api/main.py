...
from raven.api.auth import configured_key_tier, require_api_key
...
async def beta_access_gate(request: Request, call_next):
    ...
    if (
        (_BETA_MASTER and api_key == _BETA_MASTER)
...
async def security_headers(request: Request, call_next):
    """Apply least-privilege browser protections without breaking API docs."""
...
async def landing() -> HTMLResponse:
    ...
    if html.exists():
        return HTMLResponse(content=html.read_text(encoding="utf-8"), headers={"Cache-Control": "no-store, max-age=0"})
    return HTMLResponse(content="<html><body><h1>Service unavailable</h1></body></html>", headers={"Cache-Control": "no-store, max-age=0"})


@app.get("/health", tags=["ops"])
async def health() -> dict[str, str]:
    return {"status": "ok", "service": "raven-incident-api", "version": "1.0.0"}
