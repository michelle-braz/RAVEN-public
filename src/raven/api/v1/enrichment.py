...
_EVENT_PATTERNS: list[tuple[str, re.Pattern[str]]] = [
    ...
    ("authentication_failure", re.compile(
        r"login.fail|auth.fail|auth.unavailable|password|invalid.cred|\b401\b|\b403\b|ldap|oauth|\bsso\b"
        r"|unauthorized|lockout|denied|mfa|2fa|multi.factor",
        re.I,
    )),
    ...
    ("service_unavailable", re.compile(
        ...
        r"|bad.gateway|\b502\b|degradation",
        re.I,
    )),
    ("error_rate_increase", re.compile(
        r"error.rate|5xx|\b500\b|\berrors.increas|\berror.spike|exception|traceback|crash|panic|oom|out.of.memory|fatal",
        re.I,
    )),
...
def _detect_event_type(message: str) -> str:
    for name, pattern in _EVENT_PATTERNS:
        candidate = _ZERO_ERROR_RATE_RE.sub("", message) if name == "error_rate_increase" else message
...
_OBJECT_PATTERNS: list[tuple[str, re.Pattern[str]]] = [
    ("auth-service",         re.compile(r"auth[- ]?service|login.service|\bsso\b|ldap|oauth|\biam\b|identity.provider|mfa|2fa|multi.factor", re.I)),
